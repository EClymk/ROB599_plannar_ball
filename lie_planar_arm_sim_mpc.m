...
q0 = [x_0 0 y_0 0 Lb_0 0 theta1_0 0 theta2_0 0 0].';
omega_des = - (Lb_des - Lb_0) / params.r
qdes = [x_des 0 y_des 0 Lb_des 0 theta1_des 0 theta2_des 0 omega_des].';
...
for iter = 1:numel(t_sim)
    ...
    H = Hlfunc(Q,R,qdes,T);
    c = clfunc(Q,R,qdes,T);
    A = Alfunc(T, ddthetamax);
    b = blfunc(T, ddthetamax);
    Aeq = Aeqlfunc(q0,qdes,tho_vec,T);
    beq = beqlfunc(q0,qdes,tho_vec,T);
...
%     odefunparams = @(t,q) lie_planar_arm_sim_ode(t,q,[interp1(linspace(0,T,N),u_real,t)], params);
%      [tout, qout] = ode45(odefunparams,[0 Ts],q0);
%     qout, tout = integrate_lie_group_dynamics(q0,u,0.1,params,0.01)';
% @(t,q) planar_arm_sim_ode_2(t,q,[interp1(linspace(0,T,N),u_real,t)], params);
    odefunparams = @(t,q) planar_arm_sim_ode_2(t,q(1:10),[interp1(linspace(0,T,N),u_real,t)], params);
    [tout, qout] = ode45(odefunparams,[0 Ts],q0(1:10));

    qout = [qout, (qout(:,5)-Lb_0)/params.r];
...
vars={'x', 'dx', 'y', 'dy', 'Lb', 'dLb','\theta1', 'd\theta1', '\theta2','d\theta2', 'dd\theta1', 'dd\theta2', '\omega'};
