...
function q = lie_group_dynamics_update(q, u, dt, params)
...
%     % Combine dynamics for all states
    ...
    q = [g_b_new(1, 3);  % Updated x from g_b_new
     dg_b(1, 3);      % Updated x velocity dx
     g_b_new(2, 3);   % Updated y from g_b_new
     dg_b(2, 3);      % Updated y velocity dy
     Lb + dLb * dt;   % Updated Lb
     dLb + ddLb * dt; % Updated dLb
     theta1 + dtheta1 * dt; % Updated theta1
     dtheta1 + ddtheta1 * dt; % Updated dtheta1
     theta2 + dtheta2 * dt; % Updated theta2
     dtheta2 + ddtheta2 * dt; % Updated dtheta2
     phi + omega_b * dt];    % Updated ball orientation
end